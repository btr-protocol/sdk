...
Inputs (SoT): dex-evm/deployments/arc-oracle-{v2,v3}-lanes.json + the deployed oracle
addresses (5042002.deploy.v2.json / .v3.json). Run from sdk/: python3 scripts/gen-oracle-lanes.py
"""
...
def feeds_ts(d):
    ...
    for sym, f in sorted(d["feeds"].items(), key=lambda kv: kv[1]["globalIndex"]):
        ...
        rows.append(
            f"    '{sym}': {{ globalIndex: {f['globalIndex']}, expBias: {f['expBias']}, cls: '{f['cls']}'{ref} }},"
        )
    return "\n".join(rows)


v2, v3 = load("arc-oracle-v2-lanes.json"), load("arc-oracle-v3-lanes.json")
a2 = load("5042002.deploy.v2.json")["oracle"]
a3 = load("5042002.deploy.v3.json")["oracle"]

out = f"""// Oracle lane maps for the packed-slot push oracles (V2/V3), per chain.
